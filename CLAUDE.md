...
`│   ├── Pedidos_Listagem.tsx  # Página de Listagem e Detalhamento de Pedidos`  
...
Define a estrutura de dados para o gerenciamento de produtos e catálogo:  
...
Define a estrutura de dados para o gerenciamento de pedidos, itens de pedidos e transações de venda:  
...
* **Layout:** Segue o padrão estrutural das demais páginas administrativas — `Sidebar` fixa à esquerda (`<aside>`) + área de conteúdo principal (`<main>`) deslocada com `pl-72`.  
...
Página para consulta, atualização de status e visualização detalhada dos itens de cada pedido realizado.

* **Layout:** `Sidebar` fixa com navegação para a página de pedidos (`activePage="pedidos"`), container com largura máxima (`max-w-[1600px]`) e área responsiva com rolagem fluida.  
* **Composição:**  
  * `PageHeader` — Título "Listagem de Pedidos" com botão principal oculto (`hideButton={true}`).  
  * `SearchInput` — Busca interativa que filtra a lista por código do pedido, cliente ou status.  
  * `PedidoTable` — Renderiza as linhas de pedidos (`PedidoRow`) e cabeçalho (`PedidoTableHeader`).  
  * Formularío de Edição/Detalhamento (Inline/Modal View) — Exibe os dados do cliente, endereço, valor total, permite alterar o status e apresenta a tabela interna com os itens (`ItemPedido[]`), calculando subtotais e preços unitários dinamicamente.  
* **Tratamento de Dados:** Suporta variações de tipagem do objeto `Product` dentro do array `produtos_do_pedido`, prevendo propriedades padrão e legadas (ex.: `nome_produto` vs. `nome`, `preco_produto` vs. `preco`).